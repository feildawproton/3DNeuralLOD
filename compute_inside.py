...
#on my computer at least, intersections_z() is faster than intersections_z_alt()
...
#we get a warning that we are failing type inference but this works faster with the type signatures
...
#it's faster with these signatures 
#@jit("(float32, float32[:,:,:])")
...
#we could go throught the points in parallel as well
#each thread could then launch a kernel to compute intersections
#however, this is really difficult to do in python
def inside_z(points, faces, inside_results):
	for ndx in range(points.shape[0]):
		intersections = intersections_z(points[ndx], faces)
		inside_results[ndx] = intersections % 2
